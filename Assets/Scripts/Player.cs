...
public class Player : MonoBehaviour
{
    ...
    void GetInput()
    {
        ...
        iDown = Input.GetButtonDown("Get_Item");
    ...
    void Jump()
    {
        if (jDown && !isJump && !isShop &!isSwap && !isDead)
        {
    ...
    void Dodge()
    {
        ...
        isDodgeReady = DodgeCooldown < DodgeDelay;

        if (dDown && !isDodge && isDodgeReady && !isReload && !isShop && !isDead)
        {
            dodgeVec = moveVec;
            speed *= 2;
    ...
    void Swap()
    {
        if (sDown1 && (!hasWeapons[0] || equipWeaponIndex == 0) && !isDodge && !isShop && !isDead)
            return;
        if (sDown2 && (!hasWeapons[1] || equipWeaponIndex == 1) && !isDodge && !isShop && !isDead)
            return;
        if (sDown3 && (!hasWeapons[2] || equipWeaponIndex == 2) && !isDodge && !isShop && !isDead)
            return;
